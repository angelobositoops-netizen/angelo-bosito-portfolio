(function () {
  ...
  /* Server errors are normally a plain string ({ ok:false, error: "..." }),
     but a platform-level failure (e.g. the function crashing before our own
     code runs) can hand back a different shape, such as Vercel's own
     { error: { code, message } }. Never render an object directly (that's
     how you get a literal "[object Object]" on screen) -- always reduce it
     to a safe string first. */
  ...
  /* ---- Contact / inquiry form ----
     Client-side validation + a hidden honeypot field and a timing check for
     basic spam filtering. No API keys or secrets live here -- the form posts
     to our own Vercel serverless function at /api/contact, which holds the
     Resend API key server-side and sends the email. */
  (function contactForm() {
    ...
    var renderedAt = Date.now(); // used for a simple bot-timing check
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(ENDPOINT, {
        ...
        var FALLBACK = 'Something went wrong sending that. Please try again, or email me directly.';
        if (!result.ok || !result.json || result.json.ok !== true) {
          done('error', extractErrorMessage(result.json, FALLBACK));
          return;
        }
        done('success', 'Thanks \u2014 your message is on its way. I\u2019ll reply within one to two business days.');
        track('contact_submit', { method: 'api' });
        form.reset();
        renderedAt = Date.now();
      }).catch(function () {
        done('error', 'Something went wrong sending that. Please check your connection and try again, or email me directly.');
      });
    });
  })();
})();
